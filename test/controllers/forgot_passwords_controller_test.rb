...
class ForgotPasswordsControllerTest < ActionController::TestCase
  ...
  test "requesting for non-existant user" do
    post :create, params: { user: { email: 'testen@blablaablaaa.de' } }
    assert_template :new
  ...
  test "actually changing the password" do
    ...
    assert user.blocked?
    assert_difference 'ActionMailer::Base.deliveries.size', +1 do
      post :update, params: { user: { password: 'blabla', password_confirmation: 'blabla', password_reset_token: token }}
    end
  ...
  test "providing unmatching password/confirmation" do
    ...
    assert_no_difference 'ActionMailer::Base.deliveries.size' do
      post :update, params: { user: { password: 'blabla', password_confirmation: 'blabla2', password_reset_token: token }}
    end
    user = User.find(user.id)
    assert_template :edit
    assert_equal user, assigns(:user)
  ...
  test "changing with invalid token" do
    ...
    assert_no_difference 'ActionMailer::Base.deliveries.size' do
      post :update, params: { user: { password: 'blabla', password_confirmation: 'blabla2', password_reset_token: '123456789012345678901234567890af' }}
    end
    user = User.find(user.id)
    assert_template :new
    assert_nil assigns(:user)
